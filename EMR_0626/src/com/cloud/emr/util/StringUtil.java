 ...
 * 类描述：暂无 
 * 创建人：lw   
 * 创建时间：2014-3-25 上午09:55:57   
 * 修改人：lw   
 * 修改时间：2014-3-25 上午09:55:57   
 * 修改备注： 
 * @version
 */
public class StringUtil {
	...
	public static String alignRight(CharSequence cs, int width, char c) {
		...
		if (len >= width)
			return cs.toString();
		return new StringBuilder().append(dup(c, width - len)).append(cs)
				.toString();
	}
	...
	public static String alignLeft(CharSequence cs, int width, char c) {
		...
		if (length >= width)
			return cs.toString();
		return new StringBuilder().append(cs).append(dup(c, width - length))
				.toString();
	}
	...
	public static String removeFirst(String str, char c) {
		return (StringUtil.isEmpty(str) || c != str.charAt(0)) ? str : str
				.substring(1);
	}
