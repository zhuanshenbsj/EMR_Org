...
public class NewWsStepEffectiveChainHandler extends Handler {

	@Override
	public boolean HandleRequest(Context context) {
		String dataType = context.getData().getDataType();
		//判断是否是有效步数
		if(Constants.WS_stepEffective_APP_BUSINESS_TYPE.equalsIgnoreCase(dataType)){
			...
			if(data.getDataValue()!=null && data.getDataValue().size()!=0){
				...
				boolean bool = context.getObservationService().insertOrUpdateData(data.getPhone(), measureTime, 
						Constants.WS_stepEffective_APP_BUSINESS_TYPE, Constants.WS_APP_TYPE, JSONObject.fromObject(map).toString(),data.getCollectDate());
				if(!bool)
					Log.getLogger().e("NewWsStepEffectiveChainHandler save data into db error!");
				return bool;
			}
			else{
				Log.getLogger().e("NewWsStepEffectiveChainHandler datavalue is null!");
				return false;
			}
		}
		else{
